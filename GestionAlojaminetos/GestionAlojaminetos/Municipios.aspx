...
<asp:Content ID="Content3" ContentPlaceHolderID="Cuerpo" runat="server">
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:reto_gp2ConnectionString3 %>" DeleteCommand="DELETE FROM `municipios` WHERE `INDICE` = ?" InsertCommand="INSERT INTO `municipios` (`CODIGO`, `MUNICIPIO`, `INDICE`) VALUES (?, ?, ?)" ProviderName="<%$ ConnectionStrings:reto_gp2ConnectionString3.ProviderName %>" SelectCommand="SELECT * FROM `MUNICIPIOS` ORDER BY `INDICE`" UpdateCommand="UPDATE `municipios` SET `CODIGO` = ?, `MUNICIPIO` = ? WHERE `INDICE` = ?">
        <DeleteParameters>
